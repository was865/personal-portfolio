  ...
  return (
    <header className="z-999 relative">
      {/* 帯は画面の一番上から引く。iOS の PWA では時計やアンテナの下を本文が
          流れていくので、そこに背当てを置く。中身はセーフエリアと --nav-top-gap
          のぶん下げる。横幅の丸いバーになるのは sm から。 */}
      <motion.div
        className="fixed inset-x-0 top-0 mx-auto h-[calc(3.5rem+var(--safe-top)+var(--nav-top-gap))] w-full rounded-none border-0 border-b border-white/50 bg-white/80 shadow-lg shadow-black/[0.03] backdrop-blur-[0.65rem] sm:top-[calc(var(--safe-top)+1.5rem)] sm:h-[3.25rem] sm:w-[30rem] sm:rounded-full sm:border dark:border-black/40 dark:bg-gray-950/85"
        initial={{ y: -16 }}
        animate={{ y: 0 }}
      ></motion.div>
      ...
      <nav className="fixed inset-x-0 top-[calc(var(--safe-top)+var(--nav-top-gap))] mx-auto flex h-[3.5rem] w-full max-w-[30rem] items-center pl-[max(0.5rem,var(--safe-left))] pr-[max(0.5rem,var(--safe-right))] sm:top-[calc(var(--safe-top)+1.7rem)] sm:h-[initial] sm:w-[initial] sm:px-0">
